...
Original file is located at
    https://colab.research.google.com/drive/12XhaUJgqHSsQpfITc-wUWEv96-q0rfcx
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, dropout=0.5):
        '''
        ntokens = len(TEXT.vocab.stoi) # 单词表的大小  28871
        ninp emsize # 嵌入维度
        nhid # nn.TransformerEncoder 全连接网络的隐藏层维度
        nlayers # nn.TransformerEncoder 中nn.TransformerEncoderLayer的数量
        nhead # the number of heads in the multiheadattention models  多头个数
        ...
        # 一个nn.TransformerEncoderLayer
        encoder_layers = TransformerEncoderLayer(ninp, nhead, nhid, dropout)
        # nn.TransformerEncoder 包含多少个 nn.TransformerEncoderLayer
        self.transformer_encoder = TransformerEncoder(encoder_layers, nlayers)
        self.encoder = nn.Embedding(ntoken, ninp)  # 词嵌入层
        self.ninp = ninp  # 输入的维度 = embeddingsize
        self.decoder = nn.Linear(ninp, ntoken) # ninp -> ntoken

        self.init_weights()

    def _generate_square_subsequent_mask(self, sz):
        # 生成方形后续蒙版  因为要做的是一个语言模型
    ...
    def forward(self, src):
        ...
        src = self.encoder(src) * math.sqrt(self.ninp)  # 词嵌入并*维度的开方
        src = self.pos_encoder(src) # 位置编码
        output = self.transformer_encoder(src, self.src_mask)  # encoder层
        output = self.decoder(output)  # decoder层 
        return output
